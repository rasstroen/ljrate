...
<xsl:template name="backend_pages_list">
	<xsl:call-template name="backend_pages_list_level">
		...
		<xsl:with-param name="data" select="data/pages"/>
	...
	<div class="block-subtitle-{$level}">
		...
		<div class="block-subtitle-settings">
			<div class="block-subtitle-settings-header">
				<a href="#" onclick="toggle('subplank_m{@id}')">Настройки модулей страницы</a>
				<a href="#" onclick="toggle('subplank_c{@id}')">Настройки кеширования</a>
...
<xsl:template name="backend_pages_list_modules">
	...
	<xsl:for-each select="$data/modules/item[@inherited=0]">
		...
		<div id="subplank_m{$data/@id}_module_{@id}" class="module_in_page">
			<div class="onoff">
				<span>on</span>
				<input type="radio" name="modules_pages[{$data/@id}][{@id}][enabled]" checked="checked" value="1"></input>
				<span>off</span>
				<input type="radio" name="modules_pages[{$data/@id}][{@id}][enabled]"  value="0"></input>
			</div>
			<div class="onoffr">
				<div class="modtitle"><xsl:value-of select="@title" /></div>
			</div>
			<br clear="all" />
			...
			<xsl:for-each select="$roles">
				...
				<input type="checkbox" name="pmr[{$data/@id}][{$item/@id}][{@id}]">
	...
	<a href="#" onclick="showAddModule({$data/@id})">[добавить модуль]</a>
</xsl:template>
...
<xsl:template name="backend_modules_list">
	<xsl:call-template name="backend_modules_list_level">
		<xsl:with-param name="data" select="data/modules"/>
		<xsl:with-param name="level" select="0"/>
	</xsl:call-template>
</xsl:template>
